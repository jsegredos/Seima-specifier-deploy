const __vite__mapDeps=(i,m=__vite__mapDeps,d=(m.f||(m.f=["assets/index-BXlLpglg.js","assets/auth-ui-BQ2Xt-Oe.js","assets/product-search-DFc_sNRK.js","assets/vendor-fuse-Ch1WBRTM.js","assets/pdf-layouts-BftXqYv_.js","assets/data-layer-r3NNT-_8.js","assets/product-synonyms-CeNdtqLO.js","assets/vendor-idb-DwnyWBFG.js"])))=>i.map(i=>d[i]);
import"./auth-ui-BQ2Xt-Oe.js";/* empty css                      */import{_ as K,C as R}from"./config-f_a7eRcq.js";import{c as u}from"./competitor-service-D0gaRWhx.js";class A{constructor(){this._pending=[],this._filter="all",this._competitorFilter="",this._seimaByCode={}}async init(){const{authService:t}=await K(async()=>{const{authService:a}=await import("./index-BXlLpglg.js");return{authService:a}},__vite__mapDeps([0,1,2,3,4])),i=t.getCurrentUser();if(!i||!t.isStaffMode()){document.getElementById("ap-content").style.display="none",document.getElementById("ap-forbidden").style.display="";return}u.isEnabled()||u.configure(R.CROSSHAIR,i.email),await this._loadSeimaData(),await u.preload().catch(()=>{}),await this._loadPending(),this._render()}async _loadSeimaData(){try{const{dataLayer:t}=await K(async()=>{const{dataLayer:a}=await import("./data-layer-r3NNT-_8.js").then(e=>e.a);return{dataLayer:a}},__vite__mapDeps([5,1,2,3,6,7]));t.isLoaded||await t.init();const i=t.products||[];for(const a of i){const e=String(a.OrderCode||a["Order Code"]||a.Code||"").trim();e&&(this._seimaByCode[e]=a)}}catch(t){console.error("AdminPending: failed to load Seima catalog",t)}}async _loadPending(){try{const t=await u.getPendingVerifications();this._pending=t.pending||[]}catch(t){console.error("AdminPending: failed to load pending",t),this._pending=[]}}_render(){const t=document.getElementById("ap-content"),i=this._getFilteredItems(),a=this._pending.filter(o=>o.Status==="Pending-User"),e=this._pending.filter(o=>o.Status==="Pending-Dispute"),s=[...new Set(this._pending.map(o=>o.competitor))];let n="";if(n+=`
      <div class="ap-stats">
