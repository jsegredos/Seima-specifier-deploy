import{C as G,c as j}from"./competitor-service-COb0KT1S.js";import{U as S,g as Xe,p as rt,a as ae,d as yt,P as Qe,c as wt,h as vt,b as bt,e as St,f as xt,i as Et,j as Pt,k as Ct,l as It,m as Rt,n as J,o as ve}from"./pdf-layouts-RVcvTGLU.js";const x={DEBUG:"debug",INFO:"info",WARN:"warn",ERROR:"error",CRITICAL:"critical"},B={NETWORK:"network",DATA:"data",UI:"ui",PDF:"pdf",STORAGE:"storage",COMPATIBILITY:"compatibility",IMPORT:"import",VALIDATION:"validation"};class kt{constructor(){this.logs=[],this.maxLogs=1e3,this.enableConsoleLogging=!0,this.enableUserNotifications=!0,this.errorStats=new Map,this.setupGlobalErrorHandlers()}setupGlobalErrorHandlers(){window.addEventListener("error",e=>{var t;e.message&&e.message.includes("ResizeObserver")||this.handleError({message:e.message,filename:e.filename,lineNumber:e.lineno,columnNumber:e.colno,error:e.error,category:B.UI,context:"global",showUser:!((t=e.message)!=null&&t.includes("ResizeObserver"))})}),window.addEventListener("unhandledrejection",e=>{String(e.reason||"").includes("ResizeObserver")||this.handleError({message:`Unhandled promise rejection: ${e.reason}`,error:e.reason,category:B.DATA,context:"promise"})})}handleError(e){const{message:t,error:o,category:s=B.UI,context:n="unknown",level:r=x.ERROR,showUser:i=!0}=e,a={id:this.generateErrorId(),timestamp:new Date().toISOString(),message:t,category:s,context:n,level:r,stack:(o==null?void 0:o.stack)||new Error().stack,userAgent:navigator.userAgent,url:window.location.href,additionalInfo:this.gatherAdditionalInfo(o)};return this.log(a),this.updateErrorStats(s),i&&this.enableUserNotifications&&r!==x.DEBUG&&this.showUserNotification(a),r===x.CRITICAL&&this.attemptRecovery(a),a.id}log(e,t=x.INFO){const o=typeof e=="string"?{message:e,level:t,timestamp:new Date().toISOString()}:e;this.logs.push(o),this.logs.length>this.maxLogs&&this.logs.shift(),this.enableConsoleLogging&&this.consoleLog(o),(t===x.ERROR||t===x.CRITICAL)&&this.persistCriticalLog(o)}consoleLog(e){const{level:t,message:o,category:s,context:n}=e,r=`[${t.toUpperCase()}]${s?` [${s}]`:""}${n?` [${n}]`:""}`;switch(t){case x.DEBUG:console.debug(r,o,e);break;case x.INFO:console.info(r,o);break;case x.WARN:console.warn(r,o,e);break;case x.ERROR:case x.CRITICAL:console.error(r,o,e);break;default:console.log(r,o)}}showUserNotification(e){const{message:t,category:o,level:s}=e,n=this.generateUserFriendlyMessage(t,o);s!==x.CRITICAL?this.showToast(n,s):this.showErrorModal(n,e)}showToast(e,t){const o=document.createElement("div");o.className=`error-toast error-toast--${t}`,o.style.cssText=`
      position: fixed;
      top: 20px;
      right: 20px;
      z-index: 10000;
    ...
    `,document.body.appendChild(o),setTimeout(()=>o.style.transform="translateX(0)",100);const s=t===x.ERROR?8e3:5e3;setTimeout(()=>{o.parentElement&&(o.style.transform="translateX(100%)",setTimeout(()=>o.remove(),300))},s)}showErrorModal(e,t){const o=document.createElement("div");o.style.cssText=`
...
`).filter(i=>i.trim()!==""),n=s.length>0?s[s.length-1]:"Unknown",r=n.split(" - ")[0]||n;o.innerText=r,o.innerText.trim()||(o.innerText="v2.1.0")}}else throw new Error("Version file not found")}catch{const t=document.getElementById("version-number");if(t){const o=I.get("app.version")||"v2.1.0";t.innerText=o}else setTimeout(()=>{const o=document.getElementById("version-number");if(o&&!o.innerText.trim()){const s=I.get("app.version")||"v2.1.0";o.innerText=s}},1e3);console.info("Version loaded from config (GitHub Pages mode)")}}async showProductLookupScreen(){try{const t=await(await fetch("./screens/product-grid.html")).text();document.body.innerHTML=t,this.currentScreen="product-grid";const o=document.createElement("script");o.type="module",o.src="js/app.js",document.body.appendChild(o),setTimeout(()=>{document.querySelectorAll(".back-btn").forEach(s=>s.remove())},100),window.productGridManager&&window.productGridManager.init(),await this.loadVersion(),setTimeout(()=>this.loadVersion(),1e3)}catch(e){console.error("Failed to load product grid screen:",e)}}setupSplitInterface(){const e=document.getElementById("back-to-home");e&&(e.onclick=()=>location.reload());const t=document.getElementById("download-btn"),o=document.getElementById("clear-all-btn");t&&(t.onclick=()=>this.showDownloadFormModal()),o&&(o.onclick=()=>this.showClearConfirmModal()),this.setupSplitProductSearch(),this.setupReviewTable(),this.renderReviewTable(),this.loadInitialSearchResults()}setupSplitProductSearch(){const e=document.getElementById("product-search-input"),t=document.getElementById("search-results-list"),o=document.getElementById("search-loading"),s=document.getElementById("search-no-results");if(!e||!t)return;const n=[],r=S.debounce(i=>{this.performSplitProductSearch(i,t,n,o,s)},200);e.addEventListener("input",()=>{const i=e.value.trim();i?r(i):this.loadInitialSearchResults()}),t.addEventListener("click",i=>{const a=i.target.closest(".result-item");if(!a)return;const c=parseInt(a.getAttribute("data-idx"),10),l=n.length>0?n:this.currentSearchResults||[];!isNaN(c)&&l[c]&&this.showSplitProductDetails(l[c])})}performSplitProductSearch(e,t,o,s,n){if(!O.isLoaded){s.style.display="flex",n.style.display="none",t.innerHTML="";return}o.length=0,o.push(...O.searchProducts(e)),s.style.display="none",o.length===0?(n.style.display="flex",t.innerHTML=""):(n.style.display="none",t.innerHTML=o.map((r,i)=>`
        ...
        <div class="table-row" data-index="${l}">
          ...
          <div class="col-product">
            <div class="product-info">
              <div class="product-name">${S.sanitizeInput(d.Description||d.ProductName||d["Product Name"]||"")}</div>
              <div class="product-code">${S.sanitizeInput(d.OrderCode||d.Code||"")}</div>
      ...
      `}).join("")}getRoomOptions(e){let t=`<option value="Blank"${e==="Blank"?" selected":""}>Blank</option>`;return I.get("rooms.predefined",[]).forEach(n=>{t+=`<option value="${n.name}"${e===n.name?" selected":""}>${n.name}</option>`}),v.getCustomRooms().forEach(n=>{t+=`<option value="${n.name}"${e===n.name?" selected":""}>${n.name}</option>`}),t+='<option value="__ADD_NEW_ROOM__" style="font-weight: bold; color: #2563eb;">➕ Add new room...</option>',t}handleQuantityChange(e){const t=parseInt(e.getAttribute("data-index")),o=Math.max(1,parseInt(e.value)||1),s=v.getSelectedProducts();s[t]&&(s[t].quantity=o,v.setSelectedProducts(s),this.renderReviewTable(),this.updateSelectionCount())}handleRoomChange(e){const t=parseInt(e.getAttribute("data-index"));let o=e.value;if(o==="__ADD_NEW_ROOM__"){const n=prompt("Enter new room name:");if(n&&n.trim()){const r=n.trim();if(v.addCustomRoom(r)){o=r,console.log("✅ Added new room:",r),this.renderSelectionTable();return}else{alert("Room name already exists or is invalid");const i=v.getSelectedProducts();i[t]&&(e.value=i[t].room||"Blank");return}}else{const r=v.getSelectedProducts();r[t]&&(e.value=r[t].room||"Blank");return}}const s=v.getSelectedProducts();s[t]&&(s[t].room=o,v.setSelectedProducts(s),this.updateSelectionCount())}handleRemoveProduct(e){const t=parseInt(e.getAttribute("data-index")),o=v.getSelectedProducts();o[t]&&(o.splice(t,1),v.setSelectedProducts(o),this.renderReviewTable(),this.updateSelectionCount())}async showReviewScreen(){try{const t=await(await fetch("./screens/review.html")).text();document.body.innerHTML=t,this.currentScreen="review",this.setupReviewScreenHandlers(),this.renderReviewList()}catch(e){console.error("Failed to load review screen:",e)}}setupReviewScreenHandlers(){const e=document.getElementById("back-to-grid"),t=document.getElementById("add-more-btn"),o=document.getElementById("quick-pdf-btn");e&&(e.onclick=()=>this.showProductLookupScreen()),t&&(t.onclick=()=>this.showProductLookupScreen()),o&&(o.onclick=()=>this.showDownloadFormModal())}renderReviewList(){const e=document.getElementById("review-list"),t=document.getElementById("review-empty");if(!e)return;const o=v.getSelectedProducts();if(o.length===0){e.innerHTML="",t&&(t.style.display="block");return}t&&(t.style.display="none");const s={};o.forEach(n=>{const r=n.room||"Unassigned";s[r]||(s[r]=[]),s[r].push(n)}),e.innerHTML=Object.entries(s).map(([n,r])=>`
      <div class="review-room-group">
        <div class="review-room-header">${n} <span class="room-count">(${r.length})</span></div>
        ${r.map((i,a)=>{const c=i.product,l=c.Description||c.description||c.productName||c["Product Name"]||"Product",d=c.OrderCode||c.orderCode||"",p=c.Image_URL||c.imageUrl||"assets/no-image.png",g=c.RRP_EX||c["RRP EX GST"]||c.RRP_EX||c.rrpExGst||c.RRP_EXGST||c.RRP_INCGST||c["RRP INC GST"]||"0";return`
    ...
    `,document.body.appendChild(o),Fe("./assets/seima-logo.png",(n,r,i)=>{const a=JSON.parse(localStorage.getItem("selection")||"[]"),c=JSON.parse(localStorage.getItem(G.STORAGE_KEYS.SELECTED_PRODUCTS)||"[]");let l=[];c.length>0?l=c.map(m=>({...m.product,Room:m.room,Notes:m.notes,Quantity:m.quantity,Timestamp:new Date(m.timestamp).toISOString()})):l=a;const d=document.getElementById("sort-by"),p=d?d.value:"room";switch(p){case"code":l.sort((m,b)=>{const P=m.OrderCode||m.Code||"",T=b.OrderCode||b.Code||"";return P.localeCompare(T)});break;case"product":l.sort((m,b)=>{const P=m.Description||m.ProductName||"",T=b.Description||b.ProductName||"";return P.localeCompare(T)});break;case"room":default:l.sort((m,b)=>{const P=m.Room||"Blank",T=b.Room||"Blank";return P.localeCompare(T)});break}if(!l.length){alert("No products selected."),t&&(t.style.display="none");return}if(Xe()>0)console.log(`📷 Using ${Xe()} pre-cached images (skipping duplicate preload)`);else{const m=document.getElementById("pdf-processing-notification");if(m){const b=document.createElement("span");b.id="preload-progress",b.style.cssText="display: block; font-size: 12px; margin-top: 8px; color: #1e40af;",b.textContent="Loading images: 0%",m.appendChild(b)}console.log("📷 Starting image preload for",l.length,"products"),rt(l).then(b=>{const P=document.getElementById("preload-progress");P&&(P.textContent=`✓ ${b} images ready`,P.style.color="#059669")}).catch(b=>{console.warn("Image preloading error:",b)})}const f={};p==="room"?l.forEach(m=>{const b=m.Room||"Blank";f[b]||(f[b]=[]),f[b].push(m)}):f.__all__=l;const{jsPDF:y}=window.jspdf,w=new y({orientation:"landscape",unit:"pt",format:"a4",compress:!0,putOnlyUsedFonts:!0,precision:16,userUnit:1,floatPrecision:16}),h=w.internal.pageSize.getWidth(),E=w.internal.pageSize.getHeight();Fe("./assets/seima-logo.png",(m,b,P)=>{const T=ae.getCurrentUser(),H=v.getUserSettings(),U=T?{name:T.name,email:T.email,phone:T.phone,position:T.position}:H,A=localStorage.getItem("customerLogo");yt(w,{pageWidth:h,pageHeight:E,seimaLogoDataUrl:m,seimaLogoNaturalW:b,seimaLogoNaturalH:P,customerLogoDataUrl:A,userDetails:u,staffContact:U,footerHeight:Qe.footerHeight}),w.addPage(),Fe("./assets/seima-logo-white.png",(oe,de,ue)=>{const pt=u.showRrp&&!u.excludePrice,qe=!u.excludePrice,We=!u.excludeQty,mt=wt(h,{leftMargin:32,rightMargin:32,showRrp:pt,showPrice:qe,showQty:We,showTotal:qe&&We}),{colX:pe,colW:Se,headers:Te}=mt,Pe=Qe.footerHeight;et();const Ve=(R,C,z,X,$,_,D)=>{if(!C||typeof C!="string"||C.length<10||!C.startsWith("http://")&&!C.startsWith("https://")&&!C.startsWith("data:")){D&&D();return}if((k=>{if(!k||k.length<25||/\/images\/\d+$/.test(k)||k.endsWith("/0"))return!0;const L=/\.(jpg|jpeg|png|gif|webp|svg)(\?.*)?$/i.test(k),N=k.startsWith("data:");return!L&&!N})(C)){console.warn("Skipping malformed image URL:",(C==null?void 0:C.substring(0,50))+"..."),D&&D();return}if(Y.totalImages++,u.emailCompatible){Y.failedImages++,D&&D();return}const Q=Et(C);if(Q&&Q.dataUrl)try{const k=Q.width/Q.height;let L=$,N=$/k;N>_&&(N=_,L=_*k),R.addImage(Q.dataUrl,Q.format,z,X,L,N,void 0,"FAST"),Y.optimizedImages++,D&&D();return}catch{console.warn("Failed to use cached image, falling back to direct load")}let se=!1;const he=["https://wsrv.nl/?url=","https://images.weserv.nl/?url=","https://api.codetabs.com/v1/proxy?quest="];let K=0;function Z(){if(se)return;const k=new Image;k.crossOrigin="Anonymous";let L=null;k.onload=function(){if(!se){se=!0,L&&clearTimeout(L);try{const ne=ct(0),ke=ne.imageMaxWidth,we=$,ee=_;try{const M=document.createElement("canvas"),q=M.getContext("2d"),{width:V,height:Je}=Yt(k.width,k.height,ke);M.width=V,M.height=Je,q.imageSmoothingEnabled=!0,q.imageSmoothingQuality="high",q.drawImage(k,0,0,V,Je);let Ne,$e="JPEG";const ht=Vt(M,q),gt=Wt(k);ht||gt?(Ne=M.toDataURL("image/png",ne.imageQuality),$e="PNG"):(Ne=M.toDataURL("image/jpeg",ne.imageQuality),$e="JPEG");const ft=`img_${oo(C)}`;R.addImage(Ne,$e,z,X,we,ee,ft,"FAST"),Y.optimizedImages++,D&&D()}catch(M){console.warn(`Failed to optimize image: ${C}`,M),console.warn("Error details:",M.message,M.stack);try{R.addImage(k,"JPEG",z,X,we,ee),Y.optimizedImages++,D&&D()}catch(q){console.error(`Fallback also failed for: ${C}`,q),Y.failedImages++,D&&D()}}}catch(ne){console.warn("Failed to add image to PDF:",ne),Y.failedImages++,D&&D()}}},k.onerror=function(){se||(L&&clearTimeout(L),console.warn(`Failed to load image with proxy ${K}: ${C}`),console.warn(`Error details for: ${C} - Proxy: ${he[K]}`),K++,K<he.length?setTimeout(()=>{Z()},200):(se=!0,console.warn("All proxies failed, skipping image"),Y.failedImages++,D&&D()))},L=setTimeout(()=>{se||(console.warn(`⏰ Timeout with proxy ${K}: ${C}`),k.src="",k.onload=null,k.onerror=null,K++,K<he.length?setTimeout(()=>{Z()},200):(se=!0,console.warn("All proxies timed out, skipping image"),Y.failedImages++,D&&D()))},3e3);let N=C;K<he.length&&(N=he[K]+encodeURIComponent(C)),k.src=N}Z()},W=[];Object.keys(f).forEach((R,C)=>{const z=f[R];if(!z||!Array.isArray(z)){console.warn("⚠️ Skipping invalid room items:",R,z);return}z.forEach((X,$)=>{if(!X){console.warn("⚠️ Skipping null item in room:",R,"at index:",$);return}W.push({item:X,room:R,rIdx:C,iIdx:$,isFirstInRoom:$===0,roomCount:z.length})})}),W.reduce((R,C)=>{if(!C||!C.item)return console.warn("⚠️ Skipping null row in data analysis:",C),R;const z=String(C.item.Description||""),X=String(C.item.LongDescription||""),$=String(C.item.Notes||""),_=String(C.item.OrderCode||"");return R+z.length+X.length+$.length+_.length},0);let xe=0,Ce=0;const Ie=4,Re=8,Be=Math.floor((E-80)/Ie);let Ye=Pe+8;function Ae(){if(!W||!Array.isArray(W)){console.error("❌ Critical error: rowsToDraw is not a valid array:",W),Ze(new Error("Invalid product data structure"),"generating PDF","unknown.pdf");return}if(xe>=W.length){const $=w.internal.getNumberOfPages()-1;for(let L=2;L<=$+1;L++){w.setPage(L);const ne=(L-2)*Ie,ke=Math.min(ne+Ie,W.length);let we=!1;for(let ee=ne;ee<ke;ee++)if(W[ee]&&W[ee].item&&vt(W[ee].item)){we=!0;break}bt(w,{pageWidth:h,colX:pe,colW:Se,leftMargin:32,footerHeight:Pe,logoDataUrl:oe,logoNaturalW:de,logoNaturalH:ue,headers:Te,userDetails:u,skipWelsHeader:!we}),St(w,{pageWidth:h,pageHeight:E,leftMargin:32,footerHeight:Pe,pageNumber:L-1,totalPages:$})}const _=new Date,D=String(_.getDate()).padStart(2,"0"),me=String(_.getMonth()+1).padStart(2,"0"),Q=String(_.getFullYear()).slice(-2),se=String(_.getHours()).padStart(2,"0"),he=String(_.getMinutes()).padStart(2,"0"),Z=`${u.project.replace(/[^a-zA-Z0-9\s]/g,"")}-${D}${me}${Q}.${se}${he}.pdf`,k=document.getElementById("pdf-processing-notification");k&&k.remove(),so(u.emailCompatible);try{const L=w.output("blob"),N=w.output("string"),ne=N?N.match(/\/Type\s*\/XObject/g):null,ke=N?N.match(/Tj\s/g):null,we=N?N.match(/\/A\s*<</g):null;u.pdfSize=L.size;const ee=Xt(L,Z);if(u.sendEmail&&L.size>15*1024*1024){console.warn(`❌ PDF too large for email (${(L.size/1024/1024).toFixed(1)}MB), offering email-compatible version`),no(u,Z);return}const M=Jt(L,ee.settings);if(u.sendEmail&&u.email)if(u.exportCsv){const q=Z.replace(/\.pdf$/,".csv");Ke(u,q).then(V=>{window.dispatchEvent(new CustomEvent("sendEmail",{detail:{userDetails:u,pdfBlob:M,csvBlob:V}}))}).catch(V=>{console.error("Async CSV generation for email failed:",V),window.dispatchEvent(new CustomEvent("sendEmail",{detail:{userDetails:u,pdfBlob:M,csvBlob:null}}))})}else window.dispatchEvent(new CustomEvent("sendEmail",{detail:{userDetails:u,pdfBlob:M,csvBlob:null}}));else if((async()=>{const q=await io(M);je(q,Z,"PDF")})(),u.exportCsv){const q=Z.replace(/\.pdf$/,".csv");setTimeout(()=>{Ke(u,q).then(V=>{V&&je(V,q,"CSV")}).catch(V=>{console.error("CSV generation failed:",V)})},1e3)}}catch(L){console.error("PDF generation failed:",L),Ze(L,"generating PDF",Z);const N=document.getElementById("pdf-processing-notification");N&&N.remove()}t&&(t.style.display="none");return}Ce>=Ie&&(w.addPage(),Ye=Pe+8,Ce=0);const R=W[xe];if(!R||!R.item){console.warn(`⚠️  Skipping invalid row at index ${xe}:`,R),xe++,Ae();return}const C=Ye+Be*Ce;R.isFirstInRoom&&p==="room"&&R.room!=="__all__"&&xt(w,R.room,R.roomCount,32,C);const z=pe[0],X=z+90+12;Ve(w,R.item.Image_URL||"",z,C+Re+16,90,Be-Re*2,()=>{Ve(w,R.item.Diagram_URL||"",X,C+Re+16,90,Be-Re*2,()=>{const $=C+28,_=pe[1]+Se[1]/2;w.setFontSize(10),w.setTextColor("#222"),w.text(String(R.item.OrderCode||""),_,$+10,{align:"center"}),Pt(w,R.item,_,$+35);const D=Se[2]-10;Ct(w,R.item,pe[2],$+10,D,u.excludeLongDescription);const me=Te.indexOf("WELS")+1;if(me>0&&pe[me]){const Q=pe[me]+Se[me]/2;It(w,R.item,Q,$+10)}Rt(w,R.item,pe,Se,Te,$+10,{excludePrice:u.excludePrice,includeGst:u.includeGst}),xe++,Ce++,Ae()})})}Ae()})})})}function Fe(u,e){const t=new window.Image;t.crossOrigin="Anonymous",t.onload=function(){const o=document.createElement("canvas"),s=o.getContext("2d"),n=400,r=150;let i=t.width,a=t.height;if(i>n||a>r){const l=n/i,d=r/a,p=Math.min(l,d);i=Math.round(i*p),a=Math.round(a*p)}o.width=i,o.height=a,s.imageSmoothingEnabled=!0,s.imageSmoothingQuality="high",s.drawImage(t,0,0,i,a);const c=o.toDataURL("image/png",.9);e(c,i,a)},t.src=u}function zt(){if(!document.getElementById("pdf-spinner")){const u=document.createElement("div");if(u.id="pdf-spinner",u.style.display="none",u.style.position="fixed",u.style.top="0",u.style.left="0",u.style.width="100vw",u.style.height="100vh",u.style.zIndex="9999",u.style.background="rgba(255,255,255,0.7)",u.style.alignItems="center",u.style.justifyContent="center",u.innerHTML='<div style="border:6px solid #e0e0e0;border-top:6px solid #2563eb;border-radius:50%;width:54px;height:54px;animation:spin 1s linear infinite;"></div>',document.body.appendChild(u),!document.getElementById("pdf-spinner-style")){const e=document.createElement("style");e.id="pdf-spinner-style",e.innerHTML="@keyframes spin { 0% { transform: rotate(0deg);} 100% { transform: rotate(360deg);} }",document.head.appendChild(e)}}}async function Ke(u,e){return new Promise(async t=>{if(!window.Papa)try{await S.loadScript("https://cdn.jsdelivr.net/npm/papaparse@5.4.1/papaparse.min.js")}catch(r){console.error("Failed to load PapaParse:",r),t(null);return}const o=JSON.parse(localStorage.getItem("selection")||"[]"),s=JSON.parse(localStorage.getItem(G.STORAGE_KEYS.SELECTED_PRODUCTS)||"[]");let n=[];if(s.length>0?n=s.map(r=>({...r.product,Room:r.room,Notes:r.notes,Quantity:r.quantity,Timestamp:new Date(r.timestamp).toISOString()})):n=o,!n.length){t(null);return}setTimeout(()=>{const r=n.map(i=>{let a,c,l,d;const p=u.excludePrice;let g=0;if(i.UserEditedPrice!==void 0&&i.UserEditedPrice!==null&&i.UserEditedPrice!=="")g=parseFloat(i.UserEditedPrice.toString().replace(/,/g,""));else{const h=i.RRP_EX||i["RRP EX GST"]||i.RRP_EX||i.RRP_EXGST||"";g=parseFloat((h||"0").toString().replace(/,/g,""))}a=g,l="Price ea ex GST",d="Price Total ex GST",c=!isNaN(a)&&a>=0?(a*(i.Quantity||1)).toFixed(2):"";const f=i["WELS STAR"]||i.WELS_STAR||i.WELS_STAR||i.WelsStar||"",y=f&&f.toString().trim()?f.toString().replace(/[^\d.]/g,"").trim():"",w={Code:re(i.OrderCode||""),Description:re(i.Description||""),"WELS Star":re(y),Quantity:i.Quantity||1,Notes:re(i.Notes||""),Room:re(i.Room||""),"Image URL":re(i.Image_URL||""),"Diagram URL":re(i.Diagram_URL||""),"Datasheet URL":re(i.Datasheet_URL||""),"Website URL":re(i.Website_URL||"")};return w[l]=p?"0.00":a>=0?a.toFixed(2):"",w[d]=p?"0.00":c,w});setTimeout(()=>{const i=window.Papa.unparse(r,{quotes:!0,quoteChar:'"',delimiter:",",header:!0,newline:`\r
`,skipEmptyLines:!1,escapeChar:'"',transform:{value(a,c){return typeof a=="string"?a.replace(/\0/g,"").replace(/[\x00-\x08\x0B\x0C\x0E-\x1F]/g,""):a}}});u.sendEmail?setTimeout(()=>{try{const a=btoa(unescape(encodeURIComponent(i)));t({name:e,data:a,contentType:"text/csv",originalSize:i.length,base64Size:a.length})}catch(a){console.error("CSV base64 encoding failed:",a),t(new Blob([i],{type:"text/csv"}))}},0):t(new Blob([i],{type:"text/csv"}))},0)},0)})}function re(u){return typeof u!="string"&&(u=String(u)),u=u.replace(/\0/g,"").replace(/[\x00-\x08\x0B\x0C\x0E-\x1F]/g,"").replace(/\r?\n|\r/g," ").trim(),u}async function jt(u,e,t="file"){try{if("showSaveFilePicker"in window){const s=await(await window.showSaveFilePicker({suggestedName:e,types:[{description:`${t} files`,accept:{[u.type]:[`.${e.split(".").pop()}`]}}]})).createWritable();return await s.write(u),await s.close(),!0}}catch(o){console.warn("File System Access API failed:",o)}return!1}function Gt(u,e,t="file"){try{if(u.size>2*1024*1024)return console.warn("File too large for data URI method"),!1;const o=new FileReader;return o.onload=function(s){try{const n=document.createElement("a");n.href=s.target.result,n.download=e,n.style.display="none",document.body.appendChild(n),n.click(),document.body.removeChild(n)}catch(n){console.error("Data URI download failed:",n)}},o.readAsDataURL(u),!0}catch(o){return console.warn("Data URI method failed:",o),!1}}function Ht(u,e,t="file"){const o=URL.createObjectURL(u),s=document.createElement("div");s.style.cssText=`
    position: fixed; top: 0; left: 0; width: 100%; height: 100%; 
    background: rgba(0,0,0,0.8); z-index: 10001; display: flex; 
    align-items: center; justify-content: center; padding: 20px;
  ...
  `,s.appendChild(n),document.body.appendChild(s),document.getElementById("manual-download-close").onclick=()=>{URL.revokeObjectURL(o),document.body.removeChild(s)},document.getElementById("manual-download-retry").onclick=()=>{URL.revokeObjectURL(o),document.body.removeChild(s),setTimeout(()=>{at(u,e,t)},1e3)},document.getElementById("copy-url-btn").onclick=()=>{const r=document.getElementById("manual-download-url");r.select(),r.setSelectionRange(0,99999);try{navigator.clipboard.writeText(o).then(()=>{const i=document.getElementById("copy-url-btn");i.textContent="Copied!",i.style.background="#059669",setTimeout(()=>{i.textContent="Copy",i.style.background="#059669"},2e3)}).catch(()=>{document.execCommand("copy");const i=document.getElementById("copy-url-btn");i.textContent="Copied!",setTimeout(()=>i.textContent="Copy",2e3)})}catch{alert("Copy failed. Please select the URL manually and copy it.")}},s.onclick=r=>{r.target===s&&(URL.revokeObjectURL(o),document.body.removeChild(s))},setTimeout(()=>{s.parentElement&&(URL.revokeObjectURL(o),document.body.removeChild(s))},5*60*1e3)}async function at(u,e,t="file"){try{if(await qt(u,e))return}catch(o){console.warn("Standard download failed:",o)}await jt(u,e,t)||Gt(u,e,t)||Ht(u,e,t)}function qt(u,e){return new Promise(t=>{try{const o=URL.createObjectURL(u),s=document.createElement("a");s.href=o,s.download=e,s.style.display="none",document.body.appendChild(s);const n=setTimeout(()=>{r(),t(!1)},3e3),r=()=>{clearTimeout(n),s.parentElement&&document.body.removeChild(s),setTimeout(()=>URL.revokeObjectURL(o),1e3)};s.onclick=()=>{r(),t(!0)},s.click(),setTimeout(()=>{r(),t(!0)},500)}catch(o){console.error("Standard download error:",o),t(!1)}})}function Wt(u){const e=document.createElement("canvas"),t=e.getContext("2d");e.width=Math.min(100,u.width),e.height=Math.min(100,u.height),t.drawImage(u,0,0,e.width,e.height);const s=t.getImageData(0,0,e.width,e.height).data,n=new Set;for(let r=0;r<s.length;r+=4){const i=`${s[r]},${s[r+1]},${s[r+2]}`;n.add(i)}return n.size<1e3}function Vt(u,e){const o=e.getImageData(0,0,u.width,u.height).data;for(let s=3;s<o.length;s+=4)if(o[s]<255)return!0;return!1}function Yt(u,e,t){if(u<=t)return{width:u,height:e};const o=e/u;return{width:t,height:Math.round(t*o)}}function ct(u){return u>25*1024*1024?{compressionLevel:"aggressive",imageQuality:.6,imageMaxWidth:300,removeImages:!1,usePNG:!0,message:"Aggressive compression - maintaining technical diagram clarity"}:u>20*1024*1024?{compressionLevel:"high",imageQuality:.65,imageMaxWidth:350,removeImages:!1,usePNG:!0,message:"High compression - preserving technical diagram details"}:u>15*1024*1024?{compressionLevel:"medium",imageQuality:.7,imageMaxWidth:400,removeImages:!1,usePNG:!0,message:"Medium compression - optimal for technical documentation"}:u>10*1024*1024?{compressionLevel:"light",imageQuality:.75,imageMaxWidth:450,removeImages:!1,usePNG:!0,message:"Light compression - excellent technical diagram quality"}:{compressionLevel:"minimal",imageQuality:.8,imageMaxWidth:500,removeImages:!1,usePNG:!0,message:"Minimal compression - maximum technical diagram quality"}}function Jt(u,e){return u}function Xt(u,e){const t=(u.size/1048576).toFixed(2),o=ct(u.size);if(u.size>15*1024*1024){console.warn(`Large file detected (${t} MB) - exceeds typical email limit, may need email-compatible version`);const s=document.createElement("div");s.style.cssText=`
    ...
    `,s.innerHTML=`
      ...
      ">OK</button>
    `,document.body.appendChild(s),setTimeout(()=>{s.parentElement&&s.remove()},8e3)}else u.size>3*1024*1024;return{size:u.size,sizeInMB:parseFloat(t),settings:o}}function Ze(u,e="",t=""){console.error("Detailed error:",u);const o={type:Qt(u),message:u.message||"Unknown error",context:e,filename:t,timestamp:new Date().toISOString(),userAgent:navigator.userAgent},s=document.createElement("div");s.style.cssText=`
    position: fixed; top: 0; left: 0; width: 100%; height: 100%; 
  ...
  `,s.appendChild(n),document.body.appendChild(s),document.getElementById("error-close").onclick=()=>{document.body.removeChild(s)},document.getElementById("error-retry").onclick=()=>{document.body.removeChild(s),console.log("Retry requested for:",e)},document.getElementById("error-report").onclick=()=>{to(o),alert("Error details copied to clipboard. Please send this to support.")},s.onclick=r=>{r.target===s&&document.body.removeChild(s)},o}function Qt(u){var o,s;const e=((o=u.message)==null?void 0:o.toLowerCase())||"",t=((s=u.stack)==null?void 0:s.toLowerCase())||"";return e.includes("network")||e.includes("fetch")?"network":e.includes("permission")||e.includes("denied")?"permission":e.includes("memory")||e.includes("quota")?"memory":e.includes("blob")||e.includes("url")?"download":e.includes("canvas")||e.includes("image")?"rendering":t.includes("jspdf")||e.includes("pdf")?"pdf":"unknown"}function Kt(u){return{network:"Network Connection Error",permission:"Permission Required",memory:"Insufficient Memory",download:"Download Failed",rendering:"Display Error",pdf:"PDF Generation Error",unknown:"Unexpected Error"}[u]||"Error Occurred"}function Zt(u,e,t){return{network:"Unable to load required resources. Please check your internet connection and try again.",permission:`Browser permission required to save ${t}. Please allow downloads and try again.`,memory:"Not enough memory to process this large file. Try closing other browser tabs or use fewer products.",download:`Failed to download ${t}. This may be due to browser security settings or storage limitations.`,rendering:"Unable to display product images properly. Some images may be missing from the final output.",pdf:`PDF generation failed while ${e}. The file may be too large or contain problematic data.`,unknown:`An unexpected error occurred while ${e}. Please try again or contact support.`}[u]||"An unknown error has occurred."}function eo(u){const e={network:`
      ...
      </div>`};return e[u]||e.unknown}function lt(){const u=navigator.userAgent;return u.includes("Chrome")?"Chrome":u.includes("Firefox")?"Firefox":u.includes("Safari")?"Safari":u.includes("Edge")?"Edge":u.includes("SamsungBrowser")?"Samsung Internet":"Unknown"}function to(u){const e=`
...
========================
  `.trim();try{navigator.clipboard.writeText(e)}catch(t){console.error("Failed to copy error report:",t)}}let Y={totalImages:0,optimizedImages:0,failedImages:0,totalSavings:0};function oo(u){let e=0;if(u.length===0)return e.toString();for(let t=0;t<u.length;t++){const o=u.charCodeAt(t);e=(e<<5)-e+o,e=e&e}return Math.abs(e).toString(36)}function et(){Y={totalImages:0,optimizedImages:0,failedImages:0,totalSavings:0}}function so(u=!1){Y.totalImages>0}function no(u,e){const t=document.createElement("div");t.style.cssText=`
    position: fixed; top: 0; left: 0; width: 100%; height: 100%; 
    background: rgba(0,0,0,0.8); z-index: 10001; display: flex; 
    align-items: center; justify-content: center; padding: 20px;
  ...
  `,t.appendChild(o),document.body.appendChild(t),document.getElementById("email-regular-version").onclick=()=>{t.remove();const s=new CustomEvent("sendEmailRegular",{detail:{userDetails:u,originalFilename:e}});window.dispatchEvent(s)},document.getElementById("email-optimized-version").onclick=()=>{t.remove(),u.emailCompatible=!0,Ge(u)}}const ro="tipTailSettings";async function io(u){let e={};if(window._currentTipTailSettings)e=window._currentTipTailSettings,window._currentTipTailSettings=null;else try{e=JSON.parse(localStorage.getItem(ro)||"{}")}catch(a){console.warn("Could not read tipTailSettings from localStorage:",a)}const{tipAsset:t,tipUpload:o,tailAsset:s,tailUpload:n}=e;if(!t&&!o&&!s&&!n)return u;async function r(a,c,l="file"){if(c&&a)try{const d=atob(a),p=new Uint8Array(d.length);for(let g=0;g<d.length;g++)p[g]=d.charCodeAt(g);return p.buffer}catch(d){return console.warn(`⚠️ Error converting base64 to ArrayBuffer for ${l}:`,d),null}if(a)try{const d=await fetch(a);return d.ok?await d.arrayBuffer():(console.warn(`⚠️ Failed to fetch ${l} file: ${a} (${d.status} ${d.statusText})`),null)}catch(d){return console.warn(`⚠️ Error fetching ${l} file: ${a}`,d),null}return null}async function i(a,c="file",l="unknown"){if(!a)return null;try{return await PDFLib.PDFDocument.load(a)}catch(d){return console.warn(`⚠️ Failed to parse ${c} PDF: ${l}`,d),null}}try{const a=await u.arrayBuffer(),c=await PDFLib.PDFDocument.load(a),l=await PDFLib.PDFDocument.create(),[d]=await l.copyPages(c,[0]);l.addPage(d);let p=null,g=null;if(o){const h=await r(o,!0,"tip");h?(p=await i(h,"tip","uploaded file"),p||(g="The uploaded tip file is not a valid PDF or could not be loaded.")):g="Failed to process the uploaded tip file."}else if(t){const h=await r(t,!1,"tip");h?(p=await i(h,"tip",t),p||(g=`The tip file "${t.split("/").pop()}" is not a valid PDF or could not be loaded.`)):g=`The tip file "${t.split("/").pop()}" could not be found or accessed.`}if(p){const h=Array.from({length:p.getPageCount()},(m,b)=>b);(await l.copyPages(p,h)).forEach(m=>l.addPage(m))}else g&&(console.warn(`⚠️ Tip file error: ${g}`),Oe("Tip File Issue",g));if(c.getPageCount()>1){const h=Array.from({length:c.getPageCount()-1},(m,b)=>b+1);(await l.copyPages(c,h)).forEach(m=>l.addPage(m))}let f=null,y=null;if(n){const h=await r(n,!0,"tail");h?(f=await i(h,"tail","uploaded file"),f||(y="The uploaded tail file is not a valid PDF or could not be loaded.")):y="Failed to process the uploaded tail file."}else if(s){const h=await r(s,!1,"tail");h?(f=await i(h,"tail",s),f||(y=`The tail file "${s.split("/").pop()}" is not a valid PDF or could not be loaded.`)):y=`The tail file "${s.split("/").pop()}" could not be found or accessed.`}if(f){const h=Array.from({length:f.getPageCount()},(m,b)=>b);(await l.copyPages(f,h)).forEach(m=>l.addPage(m))}else y&&(console.warn(`⚠️ Tail file error: ${y}`),Oe("Tail File Issue",y));const w=await l.save({useObjectStreams:!0,addDefaultPage:!1,objectsPerTick:20});return new Blob([w],{type:"application/pdf"})}catch(a){return console.error("❌ Error during PDF merging:",a),Oe("PDF Merging Error","An error occurred while merging the PDF files. The main PDF will be generated without tip/tail content."),u}}function Oe(u,e){const t=document.createElement("div");t.style.cssText=`
  ...
  `,document.body.appendChild(t),setTimeout(()=>{t.parentElement&&t.remove()},8e3)}const be={PROXY_URL:"https://seima-ai-proxy.seima.workers.dev",APP_SECRET:"sk-proj-KRCnI_ABkeaV18bpU1dTOpXWxpKtucw",MODEL:"gpt-4o-mini",MAX_TOKENS:2e3,TEMPERATURE:.2,MAX_CATALOG_RESULTS:40},ao=`You are Fred, the Seima AI helper — a product assistant for Seima, an Australian bathroom and kitchen products company.

Help users find products in the Seima catalog. You will receive SEIMA PRODUCT DATA with each message.

RULES:
1. ONLY reference products from the provided data. Never invent products or specs.
2. Always include the real OrderCode for every product.
3. Match the correct product type — "basin mixer" is tapware, not a basin.
4. Show 1-3 best matches. Quality over quantity.

FORMAT: Be concise. Use bullet points. Always include the OrderCode.`,co=`You are a product specification parser for building/construction projects. Extract ALL product references from the provided text.
...
Return format: [{"code":"...","name":"...","brand":"...","category":"...","quantity":1,"finish":"...","section":"...","specCode":"...","style":"..."}]`;class lo{constructor(){this.conversationHistory=[],this.isProcessing=!1}async chat(e,t,o=null){var s,n,r;if(this.isProcessing)throw new Error("A request is already in progress");this.isProcessing=!0;try{const i=[{role:"system",content:ao}],a=this._findRelevantProducts(e,t);if(a&&i.push({role:"system",content:`SEIMA PRODUCT DATA (${a.count} products):
${a.text}`}),(o==null?void 0:o.length)>0){const d=o.map(p=>[p.OrderCode,p.ProductName||p.Description,p.room?`Room: ${p.room}`:null].filter(Boolean).join(" | ")).join(`
`);i.push({role:"system",content:`USER'S CURRENT SELECTION (${o.length} products):
${d}`})}i.push(...this.conversationHistory),i.push({role:"user",content:e});const c=await this._callProxy("/v1/chat/completions",{messages:i,model:be.MODEL,max_tokens:be.MAX_TOKENS,temperature:be.TEMPERATURE}),l=(r=(n=(s=c.choices)==null?void 0:s[0])==null?void 0:n.message)==null?void 0:r.content;if(!l)throw new Error("No response from AI");return this.conversationHistory.push({role:"user",content:e},{role:"assistant",content:l}),this.conversationHistory.length>16&&(this.conversationHistory=this.conversationHistory.slice(-16)),{content:l,usage:c.usage}}finally{this.isProcessing=!1}}async extractFromSpecText(e){var n,r,i;const t=await this._callProxy("/v1/chat/completions",{model:"gpt-4o-mini",messages:[{role:"system",content:co},{role:"user",content:`Extract all product references from this specification document:

${e}`}],max_tokens:16e3,temperature:.1},18e4),o=((i=(r=(n=t.choices)==null?void 0:n[0])==null?void 0:r.message)==null?void 0:i.content)||"[]";let s;try{const a=o.replace(/^```json?\n?/i,"").replace(/\n?```$/i,"").trim();s=JSON.parse(a)}catch{console.warn("Failed to parse extraction result:",o),s=[]}return{products:s,usage:t.usage}}async extractFromSpec(e,t){const o=await this._callProxy("/v1/extract-products",{content:e,mimeType:t},18e4);return{products:o.products||[],usage:o.usage}}async crossReferenceProducts(e,t){if(!j.isEnabled())try{await j.preload()}catch{}const o=[];for(const s of e){const n={...s,status:"unmatched",seimaMatches:[],seimaProduct:null},r=(s.brand||"").toLowerCase();if((r==="seima"||r==="siema")&&s.code&&t){const i=t.findProductByCode(s.code);if(i){n.status="seima-own",n.seimaProduct=i,o.push(n);continue}}if(s.code&&j.isEnabled())try{const i=await j.findSeimaMatches(s.code);if(i){const a=i.matches.map(c=>c.SeimaSKU);n.status="verified",n.seimaMatches=i.matches,t&&a.length>0&&(n.seimaProduct=t.findProductByCode(a[0])),o.push(n);continue}}catch{}if(j.isEnabled()){const i=[s.code,s.name,s.brand].filter(Boolean).join(" ");if(i.length>=2)try{const a=await j.findAlternatives(i,3);if(a.length>0){n.status="alternative",n.seimaMatches=a.map(c=>c.match),t&&(n.seimaProduct=t.findProductByCode(a[0].seimaSKU)),o.push(n);continue}}catch{}}if(t&&s.category){const i=[s.category,s.finish].filter(Boolean).join(" "),a=t.searchProducts(i,3);a.length>0&&(n.status="catalog-suggestion",n.seimaProduct=a[0],n.catalogSuggestions=a)}o.push(n)}return o}async _callProxy(e,t,o=12e4){const s=new AbortController,n=setTimeout(()=>s.abort(),o);try{const r=await fetch(`${be.PROXY_URL}${e}`,{method:"POST",headers:{"Content-Type":"application/json",Authorization:`Bearer ${be.APP_SECRET}`},body:JSON.stringify(t),signal:s.signal});if(!r.ok){const i=await r.json().catch(()=>({}));throw new Error(i.details||i.error||`Request failed (${r.status})`)}return r.json()}catch(r){throw r.name==="AbortError"?new Error("Request timed out. The file may be too large — try a shorter document or individual pages."):r}finally{clearTimeout(n)}}_findRelevantProducts(e,t){if(!t||!t.products||t.products.length===0)return null;const o=new Map,s=this._extractTerms(e.toLowerCase());for(const r of s){const i=t.searchProducts(r,20);for(const a of i)a.OrderCode&&o.set(a.OrderCode,a)}if(s.length>1){const r=t.searchProducts(s.join(" "),20);for(const i of r)i.OrderCode&&o.set(i.OrderCode,i)}if(o.size===0)for(const r of s)for(const i of t.products)[i.ProductName,i.Description,i["Long Description"],i.LongDescription,i.Group,i.SubGroup,i.Range,i.Finish,i.Colour].filter(Boolean).join(" ").toLowerCase().includes(r)&&i.OrderCode&&o.set(i.OrderCode,i);const n=[...o.values()].slice(0,be.MAX_CATALOG_RESULTS);return n.length===0?null:{text:this._formatProducts(n),count:n.length}}_extractTerms(e){const t=new Set(["a","an","the","is","are","for","and","or","in","on","at","to","of","with","from","me","i","my","do","you","can","find","show","get","what","which","please","thanks"]);return e.replace(/[^a-z0-9\s-]/g," ").split(/\s+/).filter(o=>o.length>=2&&!t.has(o))}_formatProducts(e){return e.map(t=>{const o=[t.OrderCode];t.ProductName&&o.push(t.ProductName),t["Long Description"]?o.push(t["Long Description"]):t.LongDescription?o.push(t.LongDescription):t.Description&&o.push(t.Description),t.Group&&o.push(`Group: ${t.Group}`);const s=t["RRP EX GST"]||t.RRP_EXGST||t.RRP_EX;return s&&o.push(`$${s}`),t.Finish&&o.push(`Finish: ${t.Finish}`),t.Colour&&o.push(`Colour: ${t.Colour}`),o.filter(Boolean).join(" | ")}).join(`
`)}clearHistory(){this.conversationHistory=[]}get processing(){return this.isProcessing}}const ge=new lo;class uo{constructor(){this.selectedFile=null,this.importMode="append",this.processedData=[],this.notFoundProducts=[],this.importedMetadata=null,this._specResults=null}init(){this.setupEventHandlers(),console.log("FileImportManager initialized")}setupEventHandlers(){const e=document.getElementById("import-file-btn");e&&(e.onclick=()=>this.showImportModal());const t=document.getElementById("file-drop-zone"),o=document.getElementById("file-input");t&&o&&(t.onclick=()=>o.click(),t.ondragover=l=>{l.preventDefault(),t.style.borderColor="#059669",t.style.background="#f0fdf4"},t.ondragleave=l=>{l.preventDefault(),t.style.borderColor="#ccc",t.style.background="#fafafa"},t.ondrop=l=>{l.preventDefault(),t.style.borderColor="#ccc",t.style.background="#fafafa";const d=l.dataTransfer.files;d.length>0&&this.handleFileSelection(d[0])},o.onchange=l=>{l.target.files.length>0&&this.handleFileSelection(l.target.files[0])});const s=document.getElementById("import-cancel-btn"),n=document.getElementById("import-next-btn"),r=document.getElementById("import-back-btn"),i=document.getElementById("import-process-btn"),a=document.getElementById("import-close-btn");s&&(s.onclick=()=>this.closeModal()),n&&(n.onclick=()=>this.showImportModeStep()),r&&(r.onclick=()=>this.showFileSelectionStep()),i&&(i.onclick=()=>this.processImport()),a&&(a.onclick=()=>this.closeModal()),document.querySelectorAll('input[name="import-mode"]').forEach(l=>{l.onchange=()=>{this.importMode=l.value;const d=document.getElementById("override-warning");d&&(d.style.display=this.importMode==="override"?"block":"none")}})}showImportModal(){const e=document.getElementById("file-import-modal");e&&(e.style.display="flex",this.resetModal())}closeModal(){const e=document.getElementById("file-import-modal");e&&(e.style.display="none",this.resetModal())}resetModal(){this.selectedFile=null,this.importMode="append",this.processedData=[],this.notFoundProducts=[],this._specResults=null,this.showFileSelectionStep();const e=document.getElementById("file-input");e&&(e.value="");const t=document.getElementById("selected-file-info");t&&(t.style.display="none");const o=document.getElementById("import-next-btn");o&&(o.disabled=!0);const s=document.querySelector('input[name="import-mode"][value="append"]');s&&(s.checked=!0);const n=document.getElementById("override-warning");n&&(n.style.display="none")}showFileSelectionStep(){this.hideAllSteps();const e=document.getElementById("file-selection-step");e&&(e.style.display="block")}showImportModeStep(){this.hideAllSteps();const e=document.getElementById("import-mode-step");e&&(e.style.display="block")}showProcessingStep(){this.hideAllSteps();const e=document.getElementById("import-processing-step");e&&(e.style.display="block")}showResultsStep(){this.hideAllSteps();const e=document.getElementById("import-results-step");e&&(e.style.display="block")}showSpecResultsStep(){this.hideAllSteps();const e=document.getElementById("import-spec-results-step");e&&(e.style.display="block")}hideAllSteps(){["file-selection-step","import-mode-step","import-processing-step","import-results-step","import-spec-results-step"].forEach(t=>{const o=document.getElementById(t);o&&(o.style.display="none")})}handleFileSelection(e){console.log("File selected:",e.name,e.type,e.size);const t=I.get("import.acceptedTypes",[".csv",".xlsx",".xls",".json",".pdf",".jpg",".jpeg",".png"]),o=["text/csv","application/vnd.ms-excel","application/vnd.openxmlformats-officedocument.spreadsheetml.sheet","application/json","application/pdf","image/jpeg","image/png"],s=e.name.toLowerCase(),n=t.some(l=>s.endsWith(l.toLowerCase())),r=I.get("import.maxFileSize",15*1024*1024);if(e.size>r){const l=Math.round(r/1048576);alert(`File is too large. Maximum size is ${l}MB.`);return}if(!o.includes(e.type)&&!n){alert(`Please select a valid file. Accepted formats: ${t.join(", ")}`);return}this.selectedFile=e;const i=document.getElementById("selected-file-info"),a=document.getElementById("selected-file-name"),c=document.getElementById("import-next-btn");i&&a&&c&&(a.textContent=e.name,i.style.display="block",c.disabled=!1)}async processImport(){if(!this.selectedFile){alert("No file selected");return}console.log("Starting import process with mode:",this.importMode),this.showProcessingStep();try{const e=this.selectedFile.name.toLowerCase();if(this._isSpecFile(e)){await this._processSpecFile();return}let t;if(e.endsWith(".csv"))t=await this.parseCSV(this.selectedFile);else if(e.endsWith(".json"))t=await this.parseJSON(this.selectedFile);else if(e.endsWith(".xlsx")||e.endsWith(".xls"))t=await this.parseExcel(this.selectedFile);else throw new Error("Unsupported file format");console.log("Parsed data:",t),this.importMode==="override"&&(v.clearAllSelections(),console.log("Cleared all existing data for override mode")),await this.processDataChunked(t),this.showImportResults()}catch(e){console.error("Import failed:",e),alert(`Import failed: ${e.message}`),this.showFileSelectionStep()}}_isSpecFile(e){return/\.(pdf|jpg|jpeg|png)$/.test(e)}async _processSpecFile(){const e=document.querySelector("#import-processing-step p"),t=Math.round(this.selectedFile.size/1024);try{let o;if(this.selectedFile.type==="application/pdf"||this.selectedFile.name.toLowerCase().endsWith(".pdf")){e&&(e.innerHTML=`Reading PDF...<br><small style="color:#888">${this._escapeHtml(this.selectedFile.name)} (${t}KB)</small>`);const r=await this._extractPdfText(this.selectedFile);if(r&&r.length>100)e&&(e.innerHTML=`Analysing specification text...<br><small style="color:#888">Extracted ${r.length.toLocaleString()} characters from PDF</small>`),o=await ge.extractFromSpecText(r);else{e&&(e.innerHTML='Analysing PDF with vision AI...<br><small style="color:#888">Image-based PDF — this may take 1-2 minutes</small>');const i=await this._fileToBase64(this.selectedFile);o=await ge.extractFromSpec(i,"application/pdf")}}else{e&&(e.innerHTML=`Analysing image...<br><small style="color:#888">${this._escapeHtml(this.selectedFile.name)} (${t}KB)</small>`);const r=await this._fileToBase64(this.selectedFile),i=this.selectedFile.type||"image/jpeg";o=await ge.extractFromSpec(r,i)}if(!o.products||o.products.length===0){alert("No products were found in this file. Try a clearer image or a different page."),this.showFileSelectionStep();return}e&&(e.textContent=`Found ${o.products.length} products. Cross-referencing with Seima catalog...`);const n=await ge.crossReferenceProducts(o.products,O);this._specResults=n,this._showSpecResults(n)}catch(o){console.error("Spec processing failed:",o),alert(`Spec processing failed: ${o.message}`),this.showFileSelectionStep()}}async _extractPdfText(e){try{if(!window.pdfjsLib){const r=document.createElement("script");r.src="https://cdn.jsdelivr.net/npm/pdfjs-dist@4.9.155/build/pdf.min.mjs",r.type="module";const i=document.createElement("script");i.type="module",i.textContent=`
          import * as pdfjsLib from 'https://cdn.jsdelivr.net/npm/pdfjs-dist@4.9.155/build/pdf.min.mjs';
          pdfjsLib.GlobalWorkerOptions.workerSrc = 'https://cdn.jsdelivr.net/npm/pdfjs-dist@4.9.155/build/pdf.worker.min.mjs';
          window.pdfjsLib = pdfjsLib;
          window.dispatchEvent(new Event('pdfjsReady'));
        `,document.head.appendChild(i),await new Promise((a,c)=>{const l=setTimeout(()=>c(new Error("PDF.js load timeout")),15e3);window.addEventListener("pdfjsReady",()=>{clearTimeout(l),a()},{once:!0})})}const t=await e.arrayBuffer(),o=await window.pdfjsLib.getDocument({data:t}).promise,s=[];for(let r=1;r<=o.numPages;r++){const c=(await(await o.getPage(r)).getTextContent()).items.map(l=>l.str).join(" ");c.trim()&&s.push(c.trim())}const n=s.join(`
...
`);return console.log(`PDF text extraction: ${o.numPages} pages, ${n.length} chars`),n}catch(t){return console.warn("PDF text extraction failed, will use vision API:",t),null}}_fileToBase64(e){return new Promise((t,o)=>{const s=new FileReader;s.onload=()=>t(s.result.split(",")[1]),s.onerror=()=>o(new Error("Failed to read file")),s.readAsDataURL(e)})}_showSpecResults(e){const t=e.filter(c=>["verified","seima-own","alternative","catalog-suggestion"].includes(c.status)),o=e.filter(c=>c.status==="unmatched"||c.status==="ai-suggested"),s=[...new Set(e.map(c=>c.section).filter(Boolean))],n=document.getElementById("spec-results-content");if(!n)return;n.innerHTML=`
    ...
    `;document.body.insertAdjacentHTML("beforeend",s);const n=document.getElementById("picker-confirm-dialog");n.querySelectorAll("button[data-action]").forEach(r=>{r.addEventListener("click",()=>{const i=r.dataset.action;n.remove(),i==="replace"?this.loadSelection(e,"replace"):i==="merge"&&this.loadSelection(e,"merge")})}),n.addEventListener("click",r=>{r.target===n&&n.remove()})}async loadSelection(e,t){try{this.setLoadingState(!0);const o=await ce.loadSelection(e,t);o.success?(this.hide(),this.showToast(`Loaded ${o.productCount} products (${t})`),this.onLoadCallback&&this.onLoadCallback(o)):this.showError(o.error||"Failed to load selection")}catch(o){this.showError(o.message)}finally{this.setLoadingState(!1)}}async confirmDelete(e){if(confirm(`Delete selection for "${e.customerName||"Unknown"}"?

This can be restored later.`))try{const t=await ce.deleteSelections([e.id]);t.success?(this.showToast("Selection deleted"),this.fetchAndRenderSelections()):this.showError(t.error||"Failed to delete")}catch(t){this.showError(t.message)}}async restoreSelection(e){try{const t=await ce.restoreSelections([e.id]);t.success?(this.showToast("Selection restored"),this.fetchAndRenderSelections()):this.showError(t.error||"Failed to restore")}catch(t){this.showError(t.message)}}showToast(e){const t=document.createElement("div");t.style.cssText=`
      position: fixed;
      bottom: 20px;
      left: 50%;
    ...
    `;const a=r.querySelector(".toast-close");if(a.onclick=()=>this.dismiss(n),s&&s.callback){const c=r.querySelector(".toast-action");c.onclick=()=>{s.callback(),this.dismiss(n)}}return this.container.appendChild(r),this.toasts.set(n,r),requestAnimationFrame(()=>{r.classList.add("toast-enter")}),o>0&&setTimeout(()=>this.dismiss(n),o),n}dismiss(e){const t=this.toasts.get(e);t&&(t.classList.add("toast-exit"),t.addEventListener("animationend",()=>{t.remove(),this.toasts.delete(e)}))}dismissAll(){this.toasts.forEach((e,t)=>this.dismiss(t))}success(e,t={}){return this.show({message:e,type:"success",...t})}error(e,t={}){return this.show({message:e,type:"error",duration:6e3,...t})}warning(e,t={}){return this.show({message:e,type:"warning",...t})}info(e,t={}){return this.show({message:e,type:"info",...t})}withUndo(e,t,o=5e3){return this.show({message:e,type:"info",duration:o,action:{label:"Undo",callback:t}})}}const ie=new yo;window.toast=ie;class wo{constructor(){this.activeDropdown=null,this.updatePositionHandler=null,document.addEventListener("click",e=>{e.target.closest(".global-search-dropdown")||this.hideDropdown()})}showDropdown(e,t,o,s=!1){this.hideDropdown();const n=document.createElement("ul");n.className="global-search-dropdown";const r=300,i=e.getBoundingClientRect(),a=window.innerWidth,c=window.innerHeight,l=i.width,d=c-i.bottom,p=i.top;let g=!1;d<r&&p>d&&(g=!0);let f;g?f=Math.max(8,i.top-r-8):f=Math.min(c-r-8,i.bottom+8);let y=i.left;y+l>a-8&&(y=a-l-8),y<8&&(y=8);const w={position:"fixed",top:`${f}px`,left:`${y}px`,width:`${l}px`,minWidth:`${l}px`,maxWidth:`${l}px`,background:"#fff",border:"1px solid #d1d5db",borderRadius:"8px",boxShadow:"0 10px 25px -5px rgba(0, 0, 0, 0.15), 0 8px 10px -6px rgba(0, 0, 0, 0.10)",maxHeight:`${r}px`,overflowY:"auto",overflowX:"hidden",zIndex:"10010",listStyle:"none",margin:"0",padding:"4px 0",whiteSpace:"normal",wordWrap:"break-word",display:"block",pointerEvents:"auto",transform:"none",contain:"none",isolation:"isolate"};Object.keys(w).forEach(h=>{n.style.setProperty(h,w[h],"important")}),n.dropdownHeight=r,t.length===0?n.innerHTML='<li style="padding: 12px 16px; color: #6b7280; font-style: italic; background: #fff;">No products found</li>':n.innerHTML=t.map(h=>{if(h._crosshairValidate){const A=h._crosshairValidate,oe=A.imageUrl?`<img src="${S.sanitizeInput(A.imageUrl)}" alt="" style="width:32px; height:32px; object-fit:contain; border-radius:4px; margin-right:10px; vertical-align:middle; flex-shrink:0;">`:"";return`<li data-validate-competitor="${S.sanitizeInput(A.competitor)}" data-validate-code="${S.sanitizeInput(A.productCode)}"
            ...
            <div style="flex:1; min-width:0;">
              <div style="font-weight: 600; color: #92400e;">${S.sanitizeInput(A.productName)}</div>
              <div style="color: #78716c; font-size: 11px;">${S.sanitizeInput(A.competitor)} · #${S.sanitizeInput(A.productCode)}${A.finish?" · "+S.sanitizeInput(A.finish):""} · ${A.matchCount} suggested match${A.matchCount!==1?"es":""}</div>
            </div>
            <span style="color: #d97706; font-size: 11px; font-weight: 600; white-space: nowrap; margin-left: 8px;">Review →</span>
          </li>`}const E=h.OrderCode||h.Code||"",m=h.Description||h.ProductName||h["Product Name"]||"",b=h.Image_URL||h.image_url||"",P=h._crosshairMatch,T=P?`<span style="display: inline-block; background: ${P.status==="Verified"?"#e8f5e9":"#e3f2fd"}; color: ${P.status==="Verified"?"#2e7d32":"#1565c0"}; font-size: 10px; font-weight: 600; padding: 1px 6px; border-radius: 8px; margin-left: 8px;">↔ ${S.sanitizeInput(P.competitorLabel||P.competitor)}</span>`:"",H=b?`<img src="${S.sanitizeInput(b)}" alt="" style="width:36px; height:36px; object-fit:contain; border-radius:4px; margin-right:10px; flex-shrink:0;" onerror="this.style.display='none'">`:"",U=P?Object.fromEntries(Object.entries(h).filter(([A])=>A!=="_crosshairMatch")):h;return`<li data-product='${JSON.stringify(U).replace(/'/g,"&apos;")}'
                     style="padding: 8px 16px; cursor: pointer; border-bottom: 1px solid #f3f4f6; 
                            ...
                            white-space: normal; word-wrap: break-word; overflow: visible; background: ${P?"#f0f7ff":"#fff"} !important;">
          ${H}
          <div style="flex:1; min-width:0;">
          ...
          ${t&&!t._placeholder?`
            <div class="grid-product-display">
              <div class="grid-product-name">
                <strong>${S.sanitizeInput(r)}</strong> ${S.sanitizeInput(s)}
              </div>
            </div>
          `:`
    ...
    `;this.injectSaveDialogStyles(),document.body.insertAdjacentHTML("beforeend",n);const r=document.getElementById("save-dialog"),i=document.getElementById("save-doc-name"),a=document.getElementById("save-notes");i==null||i.focus(),i==null||i.select(),r.querySelectorAll("button[data-action]").forEach(l=>{l.addEventListener("click",async()=>{const d=l.dataset.action;if(d==="cancel"){r.remove();return}const p=(i==null?void 0:i.value.trim())||"Untitled Selection",g=(a==null?void 0:a.value.trim())||"";this.currentSelectionName=p;const f={...this.prepareSelectionData(),documentName:p,notes:g};r.querySelectorAll("button").forEach(y=>y.disabled=!0),l.textContent="Saving...";try{let y;d==="save-update"?y=await le.updateSelection(this.currentSelectionId,f):(y=await le.saveSelection(f),y.success&&y.id&&(this.currentSelectionId=y.id)),r.remove(),y.success?(this.hasUnsavedChanges=!1,this.lastSaveTime=new Date,this.updateContextHeader(),ie.success(d==="save-update"?"Selection updated!":"Selection saved!")):ie.error("Failed to save: "+(y.error||"Unknown error"))}catch(y){r.remove(),ie.error("Failed to save: "+y.message)}})}),r.addEventListener("click",l=>{l.target===r&&r.remove()});const c=l=>{l.key==="Escape"&&(r.remove(),document.removeEventListener("keydown",c))};document.addEventListener("keydown",c)}injectSaveDialogStyles(){if(document.getElementById("save-dialog-styles"))return;document.head.insertAdjacentHTML("beforeend",`
      <style id="save-dialog-styles">
        .save-dialog-overlay {
          position: fixed;
          top: 0;
    ...
    `)}showLoadPicker(){J.requireAuth(e=>{this._showLoadPickerInternal(e)})}_showLoadPickerInternal(e){dt.show(t=>{console.log("✅ Selection loaded:",t),this.currentSelectionId=t.id||null,this.currentSelectionName=t.documentName||t.customerName||"Loaded Selection",this.hasUnsavedChanges=!1,this.loadExistingProducts(),this.updateTotals(),this.updateContextHeader(),t.roomOrder&&Array.isArray(t.roomOrder)&&(this.customRoomOrder=t.roomOrder,this.saveCustomRoomOrder()),ie.success(`Loaded ${t.productCount||this.gridRows.length} products`)})}escapeHtml(e){const t=document.createElement("div");return t.textContent=e||"",t.innerHTML}}const ze="onboardingCompleted",st=2;class bo{constructor(){this.currentStep=0,this.overlay=null}shouldShow(){const e=localStorage.getItem(ze);if(!e)return!0;try{return JSON.parse(e).version<st}catch{return!0}}show(){this.shouldShow()&&this.showForced()}showForced(){const e=document.getElementById("onboarding-overlay");e&&e.remove(),this.currentStep=0,this.createOverlay(),this.renderStep()}createOverlay(){var e,t;this.overlay=document.createElement("div"),this.overlay.id="onboarding-overlay",this.overlay.innerHTML=`
    ...
    `,document.head.appendChild(e)}getSteps(){return[{icon:"logo",title:"Welcome to Product Presenter",text:"Create beautiful PDF presentations of Seima products for your clients in minutes.",features:[]},{icon:"📦",title:"Add Your Products",text:"There are three ways to get started:",features:[{icon:"📁",title:"Import a File",desc:"Upload CSV or Excel files with product codes"},{icon:"📂",title:"Load a Selection",desc:"Continue from a previous saved selection"},{icon:"🔍",title:"Search Products",desc:"Search and add products one by one"}]},{icon:"🏠",title:"Organise by Room",text:"Group products by room or area. Drag to reorder, and easily manage your selection.",features:[{icon:"🎨",title:"Colour-coded",desc:"Rooms are visually distinct for quick reference"},{icon:"📊",title:"Sort Options",desc:"Sort by Room/Group, Product Code, or Product Name"},{icon:"💾",title:"Auto-saves",desc:"Your work is automatically preserved"}]},{icon:"📄",title:"Create Your PDF",text:'Click "Create PDF" to customise and generate a professional presentation with your branding.',features:[{icon:"💰",title:"Pricing Options",desc:"Show RRP, add GST, or hide pricing entirely"},{icon:"📝",title:"Content Control",desc:"Include descriptions and custom notes"},{icon:"📑",title:"Cover Pages",desc:"Add branded cover and appendix pages"}]}]}renderStep(){const e=this.getSteps(),t=e[this.currentStep],o=document.getElementById("onboarding-content"),s=document.getElementById("onboarding-dots"),n=document.getElementById("onboarding-next");if(!o||!s)return;let r="";t.features.length>0&&(r=t.features.map(a=>`
      ...
      ${r}
    `,s.innerHTML=e.map((a,c)=>`<div class="onboarding-dot ${c===this.currentStep?"active":""}"></div>`).join(""),n&&(n.textContent=this.currentStep===e.length-1?"Get Started":"Next")}nextStep(){const e=this.getSteps();this.currentStep<e.length-1?(this.currentStep++,this.renderStep()):this.complete()}complete(){localStorage.setItem(ze,JSON.stringify({version:st,completedAt:new Date().toISOString()})),this.overlay&&(this.overlay.style.animation="fadeIn 0.2s ease reverse",setTimeout(()=>{this.overlay.remove(),this.overlay=null},200))}reset(){localStorage.removeItem(ze)}}const ut=new bo;class So{constructor(){this.panel=null,this.messagesContainer=null,this.input=null,this.sendBtn=null,this.isOpen=!1}init(){this._createPanel(),this._bindEvents()}_createPanel(){const e=document.createElement("div");e.id="ai-chat-panel",e.className="ai-chat-panel",e.innerHTML=`
    ...
    `,document.body.appendChild(e),this.panel=e,this.messagesContainer=e.querySelector("#ai-chat-messages"),this.input=e.querySelector("#ai-chat-input"),this.sendBtn=e.querySelector("#ai-chat-send"),this._showWelcome()}_bindEvents(){this.panel.querySelector(".ai-chat-close-btn").addEventListener("click",()=>this.close()),this.panel.querySelector(".ai-chat-clear-btn").addEventListener("click",()=>this._clearChat()),this.input.addEventListener("input",()=>{this.sendBtn.disabled=!this.input.value.trim(),this._autoResizeInput()}),this.input.addEventListener("keydown",e=>{e.key==="Enter"&&!e.shiftKey&&(e.preventDefault(),this._send())}),this.sendBtn.addEventListener("click",()=>this._send()),document.addEventListener("keydown",e=>{e.key==="Escape"&&this.isOpen&&this.close()})}_showWelcome(){const e=document.createElement("div");e.className="ai-chat-welcome",e.innerHTML=`
      ...
      <div class="ai-chat-suggestions">
        <button class="ai-suggestion-btn" data-prompt="What matte black basin mixers do you have under $300?">Matte black mixers</button>
        <button class="ai-suggestion-btn" data-prompt="Show me Seima wall-mounted basins">Wall-mounted basins</button>
        <button class="ai-suggestion-btn" data-prompt="What tapware is available in brushed gold?">Brushed gold tapware</button>
        <button class="ai-suggestion-btn" data-prompt="Summarise the products in my current selection">Analyse my selection</button>
      </div>
    `,this.messagesContainer.innerHTML="",this.messagesContainer.appendChild(e),e.querySelectorAll(".ai-suggestion-btn").forEach(t=>{t.addEventListener("click",()=>{this.input.value=t.dataset.prompt,this.sendBtn.disabled=!1,this._send()})})}_autoResizeInput(){this.input.style.height="auto",this.input.style.height=Math.min(this.input.scrollHeight,120)+"px"}async _send(){const e=this.input.value.trim();if(!e||ge.processing)return;const t=this.messagesContainer.querySelector(".ai-chat-welcome");t&&t.remove(),this._addMessage("user",e),this.input.value="",this.input.style.height="auto",this.sendBtn.disabled=!0;const o=this._addThinking();try{const s=v.getSelectedProducts(),n=await ge.chat(e,O,s);o.remove(),this._addMessage("assistant",n.content)}catch(s){o.remove(),this._addMessage("error",s.message||"Something went wrong. Please try again.")}this._scrollToBottom(),this.input.focus()}_addMessage(e,t){const o=document.createElement("div");o.className=`ai-msg ai-msg-${e}`;const s=document.createElement("div");return s.className="ai-msg-bubble",e==="assistant"?(s.innerHTML=this._formatMarkdown(t),this._attachProductButtons(s)):e==="error"?s.innerHTML=`<span class="ai-msg-error">${this._escapeHtml(t)}</span>`:s.textContent=t,o.appendChild(s),this.messagesContainer.appendChild(o),this._scrollToBottom(),o}_addThinking(){const e=document.createElement("div");return e.className="ai-msg ai-msg-assistant",e.innerHTML=`
      <div class="ai-msg-bubble ai-msg-thinking">
        <span class="ai-thinking-dot"></span>
        <span class="ai-thinking-dot"></span>
        <span class="ai-thinking-dot"></span>
      </div>
    `,this.messagesContainer.appendChild(e),this._scrollToBottom(),e}_formatMarkdown(e){let t=this._escapeHtml(e);return t=t.replace(/\*\*(.*?)\*\*/g,"<strong>$1</strong>"),t=t.replace(/`([^`]+)`/g,"<code>$1</code>"),t=t.replace(/^### (.+)$/gm,"<h4>$1</h4>"),t=t.replace(/^## (.+)$/gm,"<h3>$1</h3>"),t=t.replace(/^- (.+)$/gm,"<li>$1</li>"),t=t.replace(/(<li>.*<\/li>\n?)+/g,o=>`<ul>${o}</ul>`),t=t.replace(/^\d+\. (.+)$/gm,"<li>$1</li>"),t=t.replace(/\n\n/g,"</p><p>"),t=`<p>${t}</p>`,t=t.replace(/<p><\/p>/g,""),t=t.replace(/<p>(<h[34]>)/g,"$1"),t=t.replace(/(<\/h[34]>)<\/p>/g,"$1"),t=t.replace(/<p>(<ul>)/g,"$1"),t=t.replace(/(<\/ul>)<\/p>/g,"$1"),t=t.replace(/\b(\d{6})\b/g,'<button class="ai-add-product-btn" data-code="$1" title="Add $1 to selection">$1 <span class="ai-add-icon">+</span></button>'),t}_attachProductButtons(e){e.querySelectorAll(".ai-add-product-btn").forEach(t=>{t.addEventListener("click",o=>{o.preventDefault(),this._addProductToGrid(t.dataset.code,t)})})}_addProductToGrid(e,t){const o=O.findProductByCode(e);if(!o){t.classList.add("ai-add-error"),t.title="Product not found in catalog";return}v.addProductToSelection(o,"","",1),window.productGridManager&&(window.productGridManager.loadExistingProducts(),window.productGridManager.updateTotals(),window.productGridManager.ensureAtLeastOneEmptyRow()),t.classList.add("ai-add-success"),t.disabled=!0,t.innerHTML=`${e} <span class="ai-add-icon">&#10003;</span>`,t.title="Added to selection"}_escapeHtml(e){const t=document.createElement("div");return t.textContent=e,t.innerHTML}_scrollToBottom(){requestAnimationFrame(()=>{this.messagesContainer.scrollTop=this.messagesContainer.scrollHeight})}_clearChat(){ge.clearHistory(),this._showWelcome()}open(){this.panel.classList.add("open"),this.isOpen=!0,setTimeout(()=>this.input.focus(),300)}close(){this.panel.classList.remove("open"),this.isOpen=!1}toggle(){this.isOpen?this.close():this.open()}}const nt=new So;class xo{constructor(){this.navigationManager=null,this.fileImportManager=new uo,this.productGridManager=new vo,this.isInitialized=!1,F.log("SeimaScanner application starting",x.INFO)}async init(){var e,t;try{if(F.log("Initializing application modules",x.INFO),ae.configure({googleSheetsUrl:(e=G.PRESENTATION_RECORDING)==null?void 0:e.GOOGLE_SHEETS_URL,email:G.EMAIL}),J.configure({logoSrc:"assets/seima-logo.png",brandName:"Seima",appName:"Product Presenter"}),(t=G.CROSSHAIR)!=null&&t.ENABLED){const r=ae.getCurrentUser();j.configure(G.CROSSHAIR,(r==null?void 0:r.email)||""),j.preload()}const o=ve.getCompatibilityReport();F.log(`Browser compatibility: ${o.score}% (${o.browserName})`,x.INFO),ve.shouldShowCompatibilityWarning()&&this.showCompatibilityWarning(),this.navigationManager=new Mt,await this.navigationManager.init(),this.fileImportManager.init(),this.setupGlobalEventListeners(),this.productGridManager.init(),nt.init();const s=document.getElementById("ai-chat-trigger");s&&s.addEventListener("click",()=>nt.toggle()),window.navigationManager=this.navigationManager,window.productGridManager=this.productGridManager,window.browserCompatibility=ve,window.downloadWithFallback=je,window.showPdfFormScreen=Ge,this.isInitialized=!0,F.log("Seima Scanner initialized successfully",x.INFO);const n=document.querySelector(".grid-container");return n&&n.classList.add("ready"),setTimeout(()=>{ut.show()},500),!0}catch(o){return F.handleError({message:"Failed to initialize application",error:o,category:B.UI,level:x.CRITICAL,context:"app-init"}),!1}}showCompatibilityWarning(){const e=ve.getCompatibilityReport(),t=e.recommendations;if(t.length===0)return;const o=t.filter(r=>r.type==="critical"),s=e.score<I.get("compatibility.minCompatibilityScore",70);if(o.length===0&&!s)return;const n=document.createElement("div");n.style.cssText=`
    ...
    `,document.body.insertBefore(n,document.body.firstChild)}setupGlobalEventListeners(){window.addEventListener("generatePdf",e=>{const{tipTailSettings:t,...o}=e.detail;zt(),Ge(o,t||null)}),window.addEventListener("beforeunload",()=>{}),ve.features.memoryAPI&&setInterval(()=>{const e=ve.memoryInfo;e.memoryPressure==="high"&&console.warn("High memory usage detected:",e)},6e4)}getSelectedProducts(){return v.getSelectedProducts()}clearSelection(){return v.clearAllSelections()}addProduct(e,t,o,s){return v.addProductToSelection(e,t,o,s)}updateSelectionCount(){this.navigationManager&&this.navigationManager.updateSelectionCount()}showError(e){alert(e)}}document.addEventListener("DOMContentLoaded",()=>{window.seimaScanner=new xo,window.seimaScanner.init()});window.addEventListener("DOMContentLoaded",()=>{fetch("./version.txt").then(u=>u.text()).then(u=>{const e=u.trim().split(`
`);if(e.length>0){const o=e[0].split(" - ")[0].trim(),s=document.getElementById("app-version");s&&(s.textContent=`Ver: ${o}`,s.style.cursor="pointer",s.addEventListener("click",Ro))}}),Eo(),Po()});function Eo(){const u=document.getElementById("help-btn");u&&u.addEventListener("click",()=>{Co()});const e=document.getElementById("crosshair-btn");e&&e.addEventListener("click",()=>{window.location.href="screens/validator.html"});const t=document.getElementById("quick-start-btn");t&&t.addEventListener("click",()=>{ut.showForced()})}function Po(){const u=document.getElementById("user-menu-container"),e=document.getElementById("user-menu-trigger"),t=document.getElementById("user-menu-dropdown"),o=document.getElementById("sign-in-btn"),s=document.getElementById("user-avatar"),n=document.getElementById("user-name-display");function r(c){var l,d,p,g,f;if(c){u&&(u.style.display="block"),o&&(o.style.display="none");const y=document.getElementById("crosshair-btn");y&&((l=G.CROSSHAIR)!=null&&l.ENABLED)&&(y.style.display="");const w=i(c.name);s&&(s.textContent=w),n&&(n.textContent=((d=c.name)==null?void 0:d.split(" ")[0])||"User"),t&&(t.innerHTML=`
        ...
        `,(p=document.getElementById("menu-profile"))==null||p.addEventListener("click",()=>{t.style.display="none",J.showEditProfile(h=>{v.clearUserSettings(),r(h)})}),(g=document.getElementById("menu-password"))==null||g.addEventListener("click",()=>{t.style.display="none",J.showChangePassword()}),(f=document.getElementById("menu-logout"))==null||f.addEventListener("click",()=>{t.style.display="none",v.clearUserSettings(),ae.logout(),r(null)}))}else u&&(u.style.display="none"),o&&(o.style.display="block")}function i(c){if(!c)return"?";const l=c.trim().split(" ");return l.length>=2?(l[0][0]+l[l.length-1][0]).toUpperCase():c.substring(0,2).toUpperCase()}function a(c){const l=document.createElement("div");return l.textContent=c||"",l.innerHTML}e&&t&&(e.addEventListener("click",c=>{c.stopPropagation();const l=t.style.display!=="none";t.style.display=l?"none":"block"}),document.addEventListener("click",c=>{u!=null&&u.contains(c.target)||(t.style.display="none")})),o&&o.addEventListener("click",()=>{J.showLogin(c=>{r(c)})}),ae.onAuthChange=r,r(ae.getCurrentUser())}function Co(){const u=document.getElementById("user-guide-modal"),e=document.getElementById("user-guide-content");if(!u||!e)return;e.innerHTML=Io(),u.style.display="flex";const t=document.getElementById("user-guide-close");t&&(t.onclick=()=>{u.style.display="none"}),u.onclick=s=>{s.target===u&&(u.style.display="none")};const o=s=>{s.key==="Escape"&&(u.style.display="none",document.removeEventListener("keydown",o))};document.addEventListener("keydown",o)}function Io(){return`
  ...
  `}async function Ro(){try{const u=document.getElementById("changelog-modal"),e=document.getElementById("changelog-content"),s=(await(await fetch("./version.txt")).text()).trim().split(`
